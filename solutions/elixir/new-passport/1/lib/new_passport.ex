defmodule NewPassport do
  @moduledoc """
  A module to help gaining some time on issuing a new passport
  """

  @doc """
  A sequence of steps that check if a new passport is available to issue on specific time
  """
  @spec get_new_passport(%NaiveDateTime{}, %Date{}, atom()) :: {:ok, String.t()}
  def get_new_passport(now, birthday, form) do
    with {:ok, timestamp} <- enter_building(now),
         {:ok, function} <- find_counter_information(now),
         {:ok, checksum} <- stamp_form(timestamp, function.(birthday), form) do
      {:ok, get_new_passport_number(timestamp, function.(birthday), checksum)}
    else
