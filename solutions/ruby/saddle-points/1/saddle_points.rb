...
module Grid
  def self.saddle_points(inputs)
    ...
    inputs.each_with_index do |row, idx|
      ...
      tallest_in_row_indexes.each do |tallest_in_row_index|
        ...
        result << {'row' => potential_saddle_point[0], 'column' => potential_saddle_point[1]} if is_the_smallest?(inputs, potential_saddle_point)
      end
    end
  ...
  def self.is_the_smallest?(inputs, coordinates)
